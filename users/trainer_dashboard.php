...
if (!isset($_GET['relationship_id'])) {
    header('Location: trainer_request.php');
    exit;
}
...
try {
    $stmt = $pdo->prepare("
        ...
        WHERE tr.id = ? AND (tr.trainer_id = ? OR tr.member_id = ?) AND tr.status = 'approved'
    ");
...
// 현재 사용자가 트레이너인지 확인
$is_trainer = ($relationship['trainer_id'] == $user_id);
$other_user_id = $is_trainer ? $relationship['member_id'] : $relationship['trainer_id'];
$other_user_name = $is_trainer ? $relationship['member_name'] : $relationship['trainer_name'];
$other_user_image = $is_trainer ? $relationship['member_image'] : $relationship['trainer_image'];
...
if ($_POST && $is_trainer) {
    try {
        ...
        // 페이지 새로고침으로 목록 업데이트
        header('Location: trainer_dashboard.php?relationship_id=' . $relationship_id . '&message=' . urlencode($message));
        exit;
...
<head>
    ...
    <title><?= $is_trainer ? '회원 관리' : '트레이너 대시보드' ?> - 운동 관리 시스템</title>
...
<body class="bg-light">
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <a href="trainer_request.php" class="back-btn">
                <i class="fas fa-arrow-left me-2"></i>트레이너 요청으로 돌아가기
            </a>
            <h1 class="mb-0">
                <i class="fas fa-dumbbell me-3"></i><?= $is_trainer ? '회원 관리' : '트레이너 대시보드' ?>
            </h1>
        </div>
        ...
        <?php if ($relationship): ?>
                ...
                <div class="row align-items-center">
                    <div class="col-md-6">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <?php endif; ?>
                            <div>
                                <h5 class="mb-1"><?= htmlspecialchars($other_user_name) ?></h5>
                                <p class="mb-1 opacity-75"><?= htmlspecialchars($relationship['member_email']) ?></p>
                                <span class="badge bg-light text-dark">
                                    <?= $is_trainer ? '당신의 회원' : '당신의 트레이너' ?>
                                </span>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 text-md-end">
                        <div class="row text-center">
                            <div class="col-4">
                                <h4 class="mb-1"><?= count($schedules) ?></h4>
                                <small>스케줄</small>
                            </div>
                            <div class="col-4">
                                <h4 class="mb-1"><?= count($comments) ?></h4>
                                <small>코멘트</small>
                            </div>
                            <div class="col-4">
                                <h4 class="mb-1"><?= count($assessments) ?></h4>
                                <small>평가</small>
                            </div>
                        </div>
                    </div>
                                            ...
                                            <tbody>
                                                <?php foreach ($workouts as $workout): ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?= date('H:i', strtotime($workout['start_time'])) ?> - 
                                                            <?= date('H:i', strtotime($workout['end_time'])) ?>
                                                        </td>
                                                        <td>
            ...
            <?php else: ?>
                ...
                <div class="trainer-card">
                    <div class="card-header">
                        <h5 class="mb-0">
                            <i class="fas fa-info-circle me-2"></i>트레이너 정보
                        </h5>
                    </div>
                    <div class="card-body">
                        <p class="text-muted">트레이너가 설정한 스케줄, 코멘트, 평가를 확인할 수 있습니다.</p>
                        <p class="text-muted">현재 승인된 관계 상태입니다.</p>
                    </div>
                </div>
            <?php endif; ?>
        <?php endif; ?>
